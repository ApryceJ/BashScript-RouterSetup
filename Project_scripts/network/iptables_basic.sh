...
#Clear Rules
  iptables -F
#input chain
  iptables -I INPUT -i ${devcname[0]} -p tcp --dport 22 -m state --state NEW -j ACCEPT
  iptables -I INPUT -i ${devcname[0]} -d 10.16.9.1 -p tcp --dport 22 -m state --state NEW -j ACCEPT
  iptables -I INPUT -d 10.16.9.1 -m state --state NEW -j ACCEPT
  iptables -I INPUT -i ${devcname[0]} -p OSPF -m state --state NEW -j ACCEPT
  iptables -I INPUT -i eth1 -p udp --dport 67 -m state --state NEW -j ACCEPT
  iptables -I INPUT -i ${wifidev[0]} -p udp --dport 67 -m state --state NEW -j ACCEPT
